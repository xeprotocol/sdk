...
```ts title="hold-a-lease.ts"
import { readFileSync } from 'node:fs'
import { Wallet, Xe, fromMicro, holdLease, type Address } from '@xeprotocol/sdk'
...
async function leasableProvider(): Promise<Address> {
...
```sh
MINUTES=10 node hold-a-lease.ts
```

For a fuller version that checks every step and prints `PASS`/`FAIL` lines, see
[`examples/javascript/hold-lease.mjs`](../javascript/hold-lease.mjs).
